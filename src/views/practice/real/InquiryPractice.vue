...
onMounted(async () => {
  ...
  if (!auth.userId) {
    console.log('userId가 아직 없음 (연습용 하드코딩 데이터 사용)');
    // 초기 하드코딩 거래내역 채우기
    transactions.value = getSampleTransactions(selectedAccount.value);
    return;
  }
...
const fetchTransactions = async (accountId) => {
  ...
  try {
    ...
    console.error(err);
    // API 실패 시 샘플 데이터로 대체
    const acc = accounts.value.find(a => (a.id ?? a.accountId) == accountId) || selectedAccount.value;
    transactions.value = getSampleTransactions(acc);
    hasSearched.value = true;
  }
};
...
// 연습용 하드코딩 거래 생성기
function getSampleTransactions(acc) {
  const bankCode = acc?.bankCode || '004'
  return [
    { id: 9001, type: 'income', description: '월급', amount: 2500000, date: '2025-09-05', time: '09:10:00', balance: (acc?.balance || 2000000) + 2500000, bankCode },
    { id: 9002, type: 'expense', description: '편의점 결제', amount: -5200, date: '2025-09-06', time: '13:45:21', balance: (acc?.balance || 2000000) + 2494800, bankCode },
    { id: 9003, type: 'expense', description: '관리비', amount: -120000, date: '2025-09-08', time: '10:02:10', balance: (acc?.balance || 2000000) + 2374800, bankCode },
    { id: 9004, type: 'income', description: '이자 발생', amount: 3400, date: '2025-09-10', time: '00:00:05', balance: (acc?.balance || 2000000) + 2378200, bankCode },
  ]
}
